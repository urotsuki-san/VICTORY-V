#!/usr/bin/env python3
"""Regenerate the fixed bring-up ROM used by the 138K dual-core image."""
...
def program(message: str, mailbox_offset: int, marker: int) -> list[tuple[int, str]]:
    ...
    emit(encode_i("MOVI", 5, 0, 0x20), "movi r5, 0x20")
    emit(encode_r("CINC", 12, 11, 5), "cinc c12, c11, r5")
    emit(encode_i("MOVI", 6, 0, marker), f"movi r6, 0x{marker:02x}")
    emit(encode_i("CSTW", 6, 12, 0), "cstw r6, c12, 0")
...
def module(name: str, words: list[tuple[int, str]]) -> list[str]:
    lines = [
        f"module {name} (",
    ...
    lines.extend(
        ...
        for index, (word, note) in enumerate(words)
    )
...
def render() -> str:
    ...
    lines.extend(module("vv32_bringup_rom", program("VV32-A0 ready\r\n", 0x10, 0x32)))
    lines.extend(module("vv64_bringup_rom", program("VV64-A0 ready\r\n", 0x18, 0x64)))
    return "\n".join(lines)
